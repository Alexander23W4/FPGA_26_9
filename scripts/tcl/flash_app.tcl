...
if {[catch {targets -set -filter {name =~ "ARM*#0"}} _e]} {
    ...
    puts ""
    puts "    Two known causes, check them IN THIS ORDER:"
    puts ""
    puts "    1) A STALE hw_server is holding a dead scan chain. This is very"
    puts "       common and looks exactly like a dead board: 'targets' prints"
    puts "       an EMPTY chain even though the board is powered on."
    puts "       FIX: kill the leftover server, then re-run:"
    puts "           taskkill /F /IM hw_server.exe"
    puts "           taskkill /F /IM cs_server.exe"
    puts "       (Do this first -- it fixes it most of the time and costs nothing.)"
    puts ""
    puts "    2) A previous 'rst -system' left the PS half-initialised, or the"
    puts "       board really is not responding."
